...
exports.autoload = function(app, model, config) {
    ...
    io.configure(function () {
        ...
        case "mongo":
            ...
            var server = (config.database.mongo.replset)
                            ? config.database.mongo.replset.servers[0]
                            : config.database.mongo.server;
            config.socketio.set.store = new MongoStore({host: server.host, port: server.port, db: config.database.mongo.db});
            break;
        }
